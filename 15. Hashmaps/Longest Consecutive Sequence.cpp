...
Output format:
The first and only line of output contains starting and ending element of 
the longest consecutive sequence. If the length of  the longest consecutive 
...
vector<int> longestConsecutiveIncreasingSequence(int *arr, int n) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    while (m.count(j) > 0 &&
           m[j] == true) // forward  (if present and not visited)
    {
      nsize += 1;
      m[j] = false;
      j++;
    ...
    while (m.count(j) > 0 &&
           m[j] == true) // backward   (if present and not visited)
    {
      nstart = j;
      nsize += 1;
      m[j] = false;
